...
| Make `scrape:progress` durable (Redis Streams + consumer groups) | Real infrastructure change for a low-volume, non-dollar-sensitive path; reconciliation covers the same gap at a fraction of the cost. (Moot since the Redis removal — PostgreSQL is the only stateful component, ADR 0009.) |
