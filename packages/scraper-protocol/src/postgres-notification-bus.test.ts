...
import { BasePostgresNotificationBus, type NotificationClient, type NotificationClientFactory } from './postgres-notification-bus.js';
...
function makeBus(
  ...
  bus: BasePostgresNotificationBus;
  clients: FakeClient[];
  ...
  const bus = new BasePostgresNotificationBus(logger, factory, 'postgres://test');
  return { bus, clients, factory, logger };
}

describe('BasePostgresNotificationBus', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  ...
  describe('publish', () => {
    ...
    it('rejects payloads over the 8 KB notification cap without opening a client', async () => {
      ...
      await expect(bus.publish(NOTIFICATION_CHANNELS.progress, 'x'.repeat(8001))).rejects.toThrow(
        /exceeds 8000 bytes/,
      );
    ...
    it('accepts payloads exactly at the cap', async () => {
      ...
      await expect(bus.publish(NOTIFICATION_CHANNELS.progress, 'x'.repeat(8000))).resolves.toBeUndefined();
    });
  ...
  describe('subscribe', () => {
    ...
    it('keeps dispatching to other handlers when one throws', async () => {
      const { bus, clients, logger } = makeBus();
    ...
    it('rejects unknown channels without registering a handler', async () => {
      ...
      await expect(bus.subscribe('not:a:channel' as never, vi.fn())).rejects.toThrow(
    ...
    it('survives an initial connect failure and retries via the reconnect loop', async () => {
      const { bus, clients, logger } = makeBus((client, index) => {
  ...
  describe('reconnect lifecycle', () => {
    ...
    it('keeps retrying while reconnects fail', async () => {
      ...
      await vi.advanceTimersByTimeAsync(1000);
      expect(clients[1].connect).toHaveBeenCalledOnce();
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Reconnect failed'), expect.anything());
